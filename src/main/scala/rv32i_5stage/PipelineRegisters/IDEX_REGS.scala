...
class IDEX_REGS_Output extends Bundle {
  ...
  val rs1_addr = Output(UInt(32.W))
  val rs2_addr = Output(UInt(32.W))
  val rd_addr = Output(UInt(32.W))
  val inst = Output(UInt(32.W))
  val csr_addr = Output(UInt(32.W))
  val immediates = new ImmGenOut
}
...
class IDEX_REGS extends Module {
  ...
  val rs1_addr = Reg(UInt(32.W))
  val rs2_addr = Reg(UInt(32.W))
  val rd_addr = Reg(UInt(32.W))
  val csr_addr = Reg(UInt(12.W))
  val inst = Reg(UInt(32.W))
  val immdiates_regs = new Imm_Regs
  ...
  rs1 := io.in.rs1
  rs2 := io.in.rs2
  rs1_addr := io.in.rs1_addr
  rs2_addr := io.in.rs2_addr
  rd_addr := io.in.rd_addr
  csr_addr := io.in.csr_addr
  inst := io.in.inst
  immdiates_regs.i_imm := io.in.immediates.i_type_imm
  immdiates_regs.s_imm := io.in.immediates.s_type_imm
  immdiates_regs.b_imm := io.in.immediates.b_type_imm
  immdiates_regs.u_imm := io.in.immediates.u_type_imm
  immdiates_regs.j_imm := io.in.immediates.j_type_imm
  ...
  io.out.inst := inst
  io.out.immediates.i_type_imm := immdiates_regs.i_imm
  io.out.immediates.s_type_imm := immdiates_regs.s_imm
  io.out.immediates.b_type_imm := immdiates_regs.b_imm
  io.out.immediates.u_type_imm := immdiates_regs.u_imm
  io.out.immediates.j_type_imm := immdiates_regs.j_imm
...
class Imm_Regs {
  val i_imm = Reg(UInt(32.W))
  val s_imm = Reg(UInt(32.W))
  val b_imm = Reg(UInt(32.W))
  val u_imm = Reg(UInt(32.W))
  val j_imm = Reg(UInt(32.W))
}